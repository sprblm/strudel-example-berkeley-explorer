...
const contributeDataHandler: RequestHandler = async (req: Request, res: Response): Promise<void> => {
  try {
    const { location, species, dbh, healthCondition, observationDate, photos, notes } = req.body;
...
const isWithinCampus = (location: number[]) => {
  ...
  for (let i = 0, j = campusBoundary.length - 1; i < campusBoundary.length; j = i++) {
    const xi = campusBoundary[i][0], yi = campusBoundary[i][1];
    const xj = campusBoundary[j][0], yj = campusBoundary[j][1];
