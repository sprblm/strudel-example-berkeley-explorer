...
const MapContainer: React.FC<MapContainerProps> = ({
  ...
  useEffect(() => {
    ...
    if (treeData && treeVisibility) {
      ...
      // Tree click handler
      const handleTreeClick = (e: mapboxgl.MapMouseEvent & { features?: any[] }) => {
        if (e.features && e.features.length > 0) {
          const treeFeature = e.features[0];
          const treeId = treeFeature.id || treeFeature.properties.id;
          
          console.log('Tree clicked:', treeFeature);
      ...
      const handleMapMouseEnter = (e: mapboxgl.MapMouseEvent) => {
        ...
        if (e.features && e.features.length > 0) {
          ...
          const hoveredFeatureId = e.features[0].id || e.features[0].properties.id;
  ...
  useEffect(() => {
    ...
    if (airQualityData && airQualityData.length > 0 && airQualityVisibility) {
      ...
      const handleAirQualityClick = (e: mapboxgl.MapMouseEvent & { features?: any[] }) => {
        if (e.features && e.features.length > 0) {
          const airFeature = e.features[0];
          const airId = airFeature.id || airFeature.properties.id;
          
          console.log('Air quality sensor clicked:', airFeature);
      ...
      // Mouse enter handler for hover effect
      const handleAirMouseEnter = (e: mapboxgl.MapMouseEvent) => {
        const canvas = map.getCanvas();
        if (canvas) {
          Object.assign(canvas.style, cursorPointerStyle);
        }
      ...
      return () => {
        if (map.getLayer(airLayerId)) {
          ...
          if (hoveredAirQualityIdRef.current !== null) {
            map.removeFeatureState({
              source: airSourceId,
              id: hoveredAirQualityIdRef.current,
            });
          }
          if (selectedAirQualityIdRef.current !== null) {
            map.removeFeatureState({
              source: airSourceId,
              id: selectedAirQualityIdRef.current,
            });
          }
        }
      };
  ...
  useEffect(() => {
    ...
    if (buildingData && buildingVisibility) {
      ...
      const handleBuildingClick = (e: mapboxgl.MapMouseEvent & { features?: any[] }) => {
        if (e.features && e.features.length > 0) {
