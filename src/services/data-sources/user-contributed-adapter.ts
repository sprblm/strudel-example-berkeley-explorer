...
export class UserContributedAdapter {
  ...
  public description =
    'Browse datasets contributed by users of the Berkeley Environmental Health Explorer . These datasets may include research-specific climate data, regionally-focused datasets, and specialized climate indicators not available in other repositories.';
  ...
  async searchDatasets(options: SearchOptions): Promise<SearchResult> {
    try {
      ...
      if (
        typeof options.temporal_coverage === 'object' &&
        options.temporal_coverage
      ) {
        filters.push({
          field: 'temporalCoverage',
          value: [
            options.temporal_coverage.startDate,
            options.temporal_coverage.endDate,
          ],
        });
      }
      ...
      if (
        ...
        options.temporal_resolution.length > 0
      ) {
        filters.push({
  ...
  async getDatasetDetails(datasetId: string): Promise<Repository> {
    try {
      ...
      console.error(
        `Error fetching user-contributed dataset details for ${datasetId}:`,
        error
      );
      throw error;
    }
    ...
    return [
      {
        ...
        description:
          'A comprehensive dataset analyzing the urban heat island effect across major Northeast US cities. Includes temperature data from local weather stations, satellite imagery, and urban density metrics.',
        url: '#',
        variables: ['Temperature', 'Urban Heat Island', 'Urban Density'],
        citation:
          'Johnson, A. & Smith, B. (2022). Urban Heat Island Analysis of Northeast US Cities. Journal of Urban Climate, 45(2), 112-128.',
        license: 'CC BY 4.0',
      ...
      },
      {
        ...
        description:
          'An integrated drought index dataset specifically designed for agricultural applications. Combines multiple drought indicators with crop-specific sensitivity factors to provide actionable insights for agricultural planning.',
        url: '#',
        variables: [
          'Drought Index',
          'Precipitation',
        ...
        citation:
          'García-Rodríguez, M. et al. (2023). A Global Agricultural Drought Index Dataset. Agricultural Water Management, 270, 107689.',
      ...
      },
      {
        ...
        description:
          'Localized sea level rise projections for Pacific small island developing states, incorporating recent observations and improved regional modeling. This dataset addresses the limitations of global datasets by accounting for regional oceanographic factors.',
        url: '#',
        variables: ['Sea Level Rise', 'Coastal Flooding', 'Wave Height'],
        citation:
          'Tong, L. & Fiji, M. (2021). Improved Regional Sea Level Rise Projections for Pacific Island States. Journal of Pacific Oceanography, 18(3), 224-241.',
        license: 'CC BY 4.0',
